...
import UIKit
import RxSwift
private class topRatedMovieViewModel{
    var topRatedMovies = Variable([MovieDetail]())
    ...
    func fetchData(){
        ...
        movieApi.moviesObject.asObserver().map { (movies)  in
            for item in movies.results{self.topRatedMovies.value.append(item)}
        }.subscribe().disposed(by: self.disposeBag)
        DispatchQueue.global(qos: .background).async {
            movieApi.getMovieData(type: .TopRatedMovies, pageNumber: self.currentPageNumber)
        }
...
class TopRatedMovies: UIViewController {
    private let viewModel = topRatedMovieViewModel()
    ...
    override func viewDidLoad() {
        ...
        self.viewModel.topRatedMovies.asObservable().map { (movies) in
...
extension TopRatedMovies : UITableViewDelegate , UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.viewModel.topRatedMovies.value.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "movieCell") as? MovieCell else {fatalError()}
        cell.setMovie(self.viewModel.topRatedMovies.value[indexPath.row])
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        latestViewController.setMovieId(self.viewModel.topRatedMovies.value[indexPath.row].id)
        self.navigationController?.pushViewController(latestViewController, animated: true)
    }
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.row == self.viewModel.topRatedMovies.value.count - 1 {
               self.viewModel.fetchData()
           }
       }
