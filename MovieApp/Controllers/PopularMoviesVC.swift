...
import UIKit
import RxSwift
private class popularMoviesViewModel{
    var popularMovies = Variable([MovieDetail]())
    ...
    func fetchData(){
        ...
        movieApi.moviesObject.asObserver().map { (movies)  in
            for item in movies.results{self.popularMovies.value.append(item)}
        }.subscribe().disposed(by: self.disposeBag)
        DispatchQueue.global(qos: .background).async {
            movieApi.getMovieData(type: .PopularMovies, pageNumber: self.currentPageNumber)
        }
...
class PopularMoviesVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.viewModel.popularMovies.asObservable().map { (movies) in
...
extension PopularMoviesVC : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        latestViewController.setMovieId(self.viewModel.popularMovies.value[indexPath.row].id)
        self.navigationController?.pushViewController(latestViewController, animated: true)
    }
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.row == self.viewModel.popularMovies.value.count - 1 {
            self.viewModel.fetchData()
        }
    }
